...
public abstract class CallBackProxy<T extends ApiResult<R>, R> implements IType<T> {
    ...
    public Type getType() {//CallBack代理方式，获取需要解析的Type
        ...
        if (mCallBack != null) {
            ...
            if (List.class.isAssignableFrom(HttpUtils.getClass(rawType, 0)) || Map.class.isAssignableFrom(HttpUtils.getClass(rawType, 0))) {
                typeArguments = mCallBack.getType();
            } else if (CacheResult.class.isAssignableFrom(HttpUtils.getClass(rawType, 0))) {
                Type type = mCallBack.getType();
                typeArguments = HttpUtils.getParameterizedType(type, 0);
            } else {
                Type type = mCallBack.getType();
                typeArguments = HttpUtils.getClass(type, 0);
            }
        }
